...
const textFromElementNodeToBeConvertedToBold = (elementNode, doc) => {
  const b = doc.createElement('B');
  b.innerHTML = customTextContent(elementNode, doc);
  return b.outerHTML;
};
...
const textFromNode = (node, tagsToPreserve, doc) => {
  switch (node.nodeType) {
    ...
    case NodeType.ELEMENT_NODE:
      ...
      if (textContentTagsToConvertToBold.includes(node.tagName)) {
        return textFromElementNodeToBeConvertedToBold(node, doc);
      }
      if (node.tagName === 'PRE') {
