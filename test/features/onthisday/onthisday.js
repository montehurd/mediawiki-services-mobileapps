...
describe('onthisday', function() {
    ...
    */
    ...
    it('dayPageTitleFromMonthAndDayNumberStrings returns expected title for 1 digit month and 1 digit day', () => {
        assert.deepEqual(onThisDay.testing.dayPageTitleFromMonthAndDayNumberStrings('1', '1'), 'January_1');
    });
    it('dayPageTitleFromMonthAndDayNumberStrings returns expected title for 0 padded month and 1 digit day', () => {
        assert.deepEqual(onThisDay.testing.dayPageTitleFromMonthAndDayNumberStrings('01', '1'), 'January_1');
    });
    it('dayPageTitleFromMonthAndDayNumberStrings returns expected title for 0 padded month and 0 padded day', () => {
        assert.deepEqual(onThisDay.testing.dayPageTitleFromMonthAndDayNumberStrings('01', '01'), 'January_1');
    });
    ...
    it('dayURIForRequest returns expected URI for 0 padded month and 2 digit day', () => {
        assert.deepEqual(onThisDay.testing.dayURIForRequest(REQUEST_FOR_EN_01_30), 'https://en.wikipedia.org/api/rest_v1/page/html/January_30');
    });
    it('dayURIForRequest returns expected URI for 2 digit month and 0 padded day', () => {
        assert.deepEqual(onThisDay.testing.dayURIForRequest(REQUEST_FOR_EN_12_01), 'https://en.wikipedia.org/api/rest_v1/page/html/December_1');
    });
    it('dayURIForRequest returns expected URI for 1 digit month and 1 digit day', () => {
        assert.deepEqual(onThisDay.testing.dayURIForRequest(REQUEST_FOR_EN_1_1), 'https://en.wikipedia.org/api/rest_v1/page/html/January_1');
    });


    // TEST SELECTED PAGE URI GENERATION

    it('selectedURIForRequest returns expected URI for 0 padded month and 2 digit day', () => {
        assert.deepEqual(onThisDay.testing.selectedURIForRequest(REQUEST_FOR_EN_01_30), 'https://en.wikipedia.org/api/rest_v1/page/html/Wikipedia%3ASelected_anniversaries%2FJanuary_30');
    });
    it('selectedURIForRequest returns expected URI for 2 digit month and 0 padded day', () => {
        assert.deepEqual(onThisDay.testing.selectedURIForRequest(REQUEST_FOR_EN_12_01), 'https://en.wikipedia.org/api/rest_v1/page/html/Wikipedia%3ASelected_anniversaries%2FDecember_1');
    });
    it('selectedURIForRequest returns expected URI for 1 digit month and 1 digit day', () => {
        assert.deepEqual(onThisDay.testing.selectedURIForRequest(REQUEST_FOR_EN_1_1), 'https://en.wikipedia.org/api/rest_v1/page/html/Wikipedia%3ASelected_anniversaries%2FJanuary_1');
    });
    ...
    it('getDayHTMLForRequest can fetch a day page', () => {
        return onThisDay.testing.getDayHTMLForRequest(REQUEST_FOR_EN_01_30)
            .then((html) => {
                assert.ok(html.length > 0);
            });
    });
    it('getSelectedHTMLForRequest can fetch an selected anniversaries page', () => {
        return onThisDay.testing.getSelectedHTMLForRequest(REQUEST_FOR_EN_01_30)
            .then((html) => {
                assert.ok(html.length > 0);
            });
    });
